...
import { type CanvasTool, CanvasControls } from "../components/canvas-controls";
import { CanvasEmptyState } from "../components/canvas-empty-state";
import { CanvasToolbar } from "../components/canvas-toolbar";
import { NodeKindMenu } from "../components/node-kind-menu";
import { NODE_CONFIG, type NodeKind, type Port, resolveTargetHandle } from "../constants";
...
import { type AssistantOp, useAssistantChat } from "../hooks/use-assistant-chat";
...
function serializeGraph(nodes: Node[], edges: Edge[]) {
  return {
    nodes: nodes.map((n) => {
      ...
      return n.type === "sticker" ? { ...base, width: n.width, height: n.height } : base;
    }),
    edges: edges.map((e) => ({
      id: e.id,
      source: e.source,
...
function CanvasWorkspace({ projectId, initialNodes, initialEdges }: CanvasWorkspaceProps) {
  ...
  const addImageResults = useCallback(
    (sourceId: string, results: { key: string; url: string }[], kind: NodeKind = "image") => {
  ...
  const applyAssistantOps = useCallback(
    (ops: AssistantOp[]) => {
      ...
      for (const n of graphRef.current.nodes) if (n.type) kindOf.set(n.id, n.type);
      ...
      for (const op of ops) {
        if (op.op === "add_node") {
          ...
          const handle = sk && tk ? resolveTargetHandle(sk as NodeKind, tk as NodeKind) : null;
  ...
  const assistant = useAssistantChat(projectId, getAssistantNodes, applyAssistantOps);
  ...
  useEffect(() => {
    return () => {
      if (projectId && pendingGraph.current) updateWorkflow(projectId, { graph: pendingGraph.current }).catch(() => {});
    };
  }, [projectId]);
  ...
  const isValidConnection = useCallback<IsValidConnection>(
    (conn) => {
      ...
      const handle = resolveTargetHandle(source.type as NodeKind, target.type as NodeKind, conn.targetHandle);
      if (!handle) return false;
      const port = NODE_CONFIG[target.type as NodeKind]?.inputs.find((p) => p.id === handle) as
        | Port
        | undefined;
  ...
  const handleUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        setNodeData(id, { upload_key: key, upload_name: name, upload: undefined });
  ...
  return (
    <ReactFlowProvider>
      <CanvasActionsProvider value={actions}>
        ...
        <CanvasToolbar onAddNode={addNode} onUpload={handleUpload} onAddSticker={handleAddSticker} />
