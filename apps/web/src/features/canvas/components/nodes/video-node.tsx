...
export function VideoNode({ id, data, selected }: NodeProps) {
  ...
  if (uploader && !output) {
    return (
      <NodeShell id={id} kind="video" selected={Boolean(selected)} locked={Boolean(locked)} data={data}>
        <input ref={fileInputRef} type="file" accept="video/*" onChange={handleUpload} className="hidden" />
        <button
          type="button"
          onClick={() => fileInputRef.current?.click()}
          className="flex h-36 w-full flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border bg-card/40 text-muted-foreground transition-colors hover:border-foreground/40 hover:bg-accent hover:text-foreground"
        >
