...
// template's identity (so the same caption style can appear at two layouts).
// Preview videos are public hotlink-friendly stock (test-videos.co.uk +
// samplelib) for now — the poster shows until a clip loads or if it fails.
...
export function captionCss(
	...
	if (style === "custom" && custom) {
		const fonts = {
			inter: "PoppinsCap, sans-serif",
			poppins: "PoppinsCap, sans-serif",
		...
		const shadows = [
			...
			!custom.box && !custom.shadow && strokeW === 0
		...
		return {
			base: {
				...
				fontSize: px(
					custom.size_px ? custom.size_px * 0.72 : (SIZE_PX[custom.size] ?? 12),
				),
				...
				WebkitTextStroke:
					strokeW > 0 && !custom.box
						? `${strokeW * 0.35}px ${custom.stroke?.color ?? "#000"}`
						: undefined,
				background: custom.box
					? `${custom.box_color ?? "#000000"}99`
					: undefined,
			},
		...
		case "neon":
			return {
				base: {
					...
					fontSize: px(12),
					fontWeight: 700,
					textShadow: "0 0 6px #14b8a6, 0 0 12px rgba(20,184,166,0.7)",
				},
				active: {},
				className: "",
				boxed: false,
	...
	return (
		<span
			className={cn(
				...
				css.boxed && "rounded px-1.5 py-0.5",
				css.boxed && !css.base.background && "bg-black/60",
			)}
			style={css.base}
		>
			{words.map((w, i) => (
				<span
					key={`${w}-${i}`}
					style={i === activeIndex ? css.active : undefined}
				>
					{w}{" "}
				</span>
			))}
...
export function CaptionStylePicker({
	...
	const card = (
		...
		// Video previews the real motion; the poster (or a plain img) covers the
		// gap while it loads and if it ever fails.
		const media = (className: string, style?: React.CSSProperties) =>
			video ? (
				// No poster — the `#t=0.1` fragment makes the browser show the
				// video's own first frame at rest, so hover→play doesn't visibly
				// switch from an unrelated image.
				// biome-ignore lint/a11y/useMediaCaption: decorative preview footage
				<video
					src={`${video}#t=0.1`}
					loop
					muted
				...
				<img src={img} className={className} style={style} />
			);
		return (
			<button
				key={key}
			...
			>
				...
				{blurMode ? (
					// biome-ignore lint/a11y/useAltText: blurred backdrop
					// biome-ignore lint/nursery/noImgElement: static blurred backdrop
					<img
						src={img}
				...
				{fitMode
					? media("absolute left-0 w-full object-cover", {
							top: `${topBar * 100}%`,
							height: `${videoFrac * 100}%`,
						})
				...
				{watermark ? (
					// Mirrors the free-plan render watermark (bottom-right, subtle).
					<span
						className="pointer-events-none absolute bottom-[3%] right-[5%] z-[6] text-[7px] font-semibold text-white/60 [text-shadow:1px_1px_2px_rgba(0,0,0,0.6)]"
					>
						riocut.com
					</span>
				) : null}
				{headline ? (
					<span
						className="absolute inset-x-2 z-[5] line-clamp-2 text-center text-[8px] font-extrabold uppercase leading-relaxed"
						style={{ top: fitMode ? `${topBar * 35}%` : 10 }}
					>
						{/* box-decoration-clone gives each wrapped line its own rounded
						    pill that hugs its text. */}
						<span
							className={cn(
								"box-decoration-clone rounded-[3px] px-1.5 py-0.5",
								headline.bg === "none" &&
									"[text-shadow:0_1px_2px_rgba(0,0,0,0.9)]",
							)}
							style={{
								background:
									headline.bg === "none" ? "transparent" : headline.bg,
				...
				{css ? (
					<span
						className={cn(
							...
							!fitMode &&
								(css.middle ? "top-1/2 -translate-y-1/2" : "bottom-14"),
						)}
						style={
							fitMode
								? { top: `${(0.5 + videoFrac / 2) * 100 + 2}%` }
								: undefined
						}
					>
					...
					<span className="absolute inset-0 grid place-items-center text-sm text-white/35">
						No captions
					</span>
				)}
		...
		<div>
			...
			<div className="relative">
				...
				<div
					...
					className="flex snap-x gap-3 overflow-x-auto py-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
				>
					{tab === "featured" ? (
						<>
							{FEATURED_TEMPLATES.map((t) =>
								card(
									...
									t.name,
									t.bg,
									captions && style === t.style && layout === t.layout,
									() =>
										onChange({
											captions: true,
											caption_style: t.style,
											layout: t.layout,
										}),
						...
						<>
							...
							<button
								...
								style={{ width: cw, height: ch }}
								className="grid shrink-0 snap-start place-items-center rounded-lg border-2 border-dashed border-white/15 text-muted-foreground transition-colors hover:border-teal-400/50 hover:text-teal-300"
							>
...
const matchesSwatch = (
	def: CustomCaptionStyle,
	patch: Partial<CustomCaptionStyle>,
) =>
	Object.entries(patch).every(
		([k, v]) => def[k as keyof CustomCaptionStyle] === v,
	);
...
function TemplateEditor({
	...
	const set = (patch: Partial<CustomCaptionStyle>) =>
		setDef((d) => ({ ...d, ...patch }));
	const css = captionCss("custom", def, 1.1);
	const headline = def.headline ?? {
		enabled: false,
		bg: "#FFFFFF",
		color: "#000000",
	};
	...
	const SectionSwitch = ({
		...
		>
			<span
				className={cn(
					"absolute top-0.5 size-4 rounded-full bg-white shadow transition-[left] duration-150",
	...
	return (
		<div
			className="dark fixed inset-0 z-[210] grid place-items-center bg-black/80 p-4"
			onClick={onClose}
		>
			<div
				className="flex max-h-[92vh] w-full max-w-4xl flex-col rounded-lg border border-white/10 bg-[#1a1a1a] text-foreground"
			...
			>
				...
				<div className="grid min-h-0 flex-1 md:grid-cols-[1fr_280px]">
						...
						{advanced ? (
							...
							<>
								<input
									...
									className="w-full rounded-lg border border-white/10 bg-[#161616] px-4 py-3 text-sm outline-none placeholder:text-muted-foreground/60 focus:border-teal-400/50"
								/>
								...
								<div>
									<div className="flex items-center justify-between">
										...
										<SectionSwitch
											checked={subtitlesOn}
											onChange={() => set({ subtitles: !subtitlesOn })}
										/>
									</div>
									{subtitlesOn ? (
										<div className="mt-3 space-y-3">
											<label className="flex items-center gap-2.5 text-sm text-muted-foreground">
												<Checkbox
													...
													onCheckedChange={(v) =>
														set({ position: v === true ? "bottom" : "middle" })
													}
												/>
											...
											<button
												...
												className="flex w-full items-center justify-center gap-2 rounded-lg bg-white/5 py-2.5 text-sm text-foreground/90 transition-colors hover:bg-white/10"
											>
								...
								<div>
									<div className="flex items-center justify-between">
										...
										<SectionSwitch
											...
											onChange={() =>
												set({
													headline: { ...headline, enabled: !headline.enabled },
												})
											}
										/>
									</div>
									<p className="mt-0.5 text-xs text-muted-foreground">
										Shows the clip title as a banner at the top.
									</p>
									{headline.enabled ? (
								...
								<div>
									...
									<p className="mb-2.5 text-xs text-muted-foreground">
										Overlaid top-right on every exported clip.
									</p>
									<input
										ref={logoRef}
										type="file"
									...
									{def.logo ? (
										...
										<button
											...
											className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-white/15 py-3.5 text-sm text-muted-foreground transition-colors hover:border-teal-400/50 hover:text-teal-300"
										>
											<Plus className="size-4" /> Add
										</button>
									)}
									{logoError ? (
										<p className="mt-1.5 text-xs text-amber-300">{logoError}</p>
									) : null}
								</div>
							</>
						)}
					...
					<div className="hidden flex-col items-center justify-center gap-2 border-l border-white/10 bg-[#141414] p-4 md:flex">
						<span className="rounded-lg border border-white/10 px-2.5 py-1 text-[11px] text-muted-foreground">
							9:16
						</span>
						<div
							className="relative h-[360px] w-[203px] overflow-hidden rounded-lg border border-white/10"
							style={{
								background:
									def.layout === "fit" ? (def.bg ?? "#000") : undefined,
							}}
						>
							<div
								className={cn(
									...
									def.layout === "fit"
										? "inset-x-0 top-1/2 aspect-video -translate-y-1/2"
										: "inset-0",
								)}
							>
							...
							{def.logo ? (
								...
								<img
									src={def.logo}
									className="absolute right-[5%] top-[3%] w-1/5 object-contain"
								/>
							) : null}
							{headline.enabled ? (
								<span
							...
							{subtitlesOn ? (
								<span
									className={cn(
										"absolute inset-x-2 flex justify-center",
										css.middle ? "top-1/2 -translate-y-1/2" : "bottom-[13%]",
									)}
								>
				...
				<div className="flex items-center gap-2 border-t border-white/10 p-4">
					{onDelete ? (
						<button
							type="button"
							onClick={onDelete}
							className="flex items-center gap-1.5 rounded-lg px-3 py-2.5 text-sm text-red-400/90 transition-colors hover:bg-red-400/10"
						>
					...
					<button
						type="button"
						onClick={onClose}
						className="rounded-lg px-4 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-white/5"
					>
						Cancel
					</button>
					<button
	...
	return (
		<div>
			...
			<select
				...
				onChange={(e) =>
					set({ font: e.target.value as CustomCaptionStyle["font"] })
				}
				className="mb-2.5 w-full rounded-lg border border-white/10 bg-[#161616] px-4 py-3 text-sm font-semibold outline-none"
				style={{
					fontFamily: {
						inter: "PoppinsCap",
						poppins: "PoppinsCap",
			...
			<div className="mb-2.5 flex flex-wrap items-center gap-2">
				<select
					...
					className="rounded-lg border border-white/10 bg-[#161616] px-3 py-2 text-sm outline-none"
				>
				...
				<span className="flex rounded-lg border border-white/10 bg-[#161616] p-0.5">
					<Tool
						active={(def.align ?? "center") === "left"}
						label="Align left"
					...
					<Tool
						active={(def.align ?? "center") === "center"}
						label="Align center"
				...
				<span className="flex rounded-lg border border-white/10 bg-[#161616] p-0.5">
					...
					<Tool
						active={def.underline}
						label="Underline"
						onClick={() => set({ underline: !def.underline })}
					>
			...
			<div className="flex flex-wrap items-center gap-2">
				<span className="flex rounded-lg border border-white/10 bg-[#161616] p-0.5">
					<Tool
						active={!def.uppercase}
						label="Normal case"
						onClick={() => set({ uppercase: false })}
					>
						Aa
					</Tool>
					<Tool
						active={def.uppercase}
						label="Uppercase"
						onClick={() => set({ uppercase: true })}
					>
						AB
					</Tool>
				</span>
				<label className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-[#161616] px-3 py-2 text-sm text-muted-foreground">
			...
			<div className="mt-4 border-t border-white/[0.07] pt-4">
				<select
					...
					className="w-full rounded-lg border border-white/10 bg-[#161616] px-4 py-3 text-sm outline-none"
				>
						...
						<div className="flex items-center gap-2">
							...
							<button
								...
								onClick={() =>
									set({
										stroke: strokeOn
											? { width: 0, color: def.stroke?.color ?? "#000000" }
					...
					action={
						<div className="flex items-center gap-2">
							...
							<button
								...
								onClick={() =>
									set({ highlight: highlightOn ? def.color : "#2DD4BF" })
								}
								className="grid size-8 place-items-center rounded-lg text-muted-foreground transition-colors hover:bg-white/10 hover:text-foreground"
							>
								{highlightOn ? (
