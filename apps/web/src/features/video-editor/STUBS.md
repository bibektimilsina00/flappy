...
- [x] **Magic B-Roll** — transcribe → group into topic windows → fetch a stock photo per topic (`POST .../broll`); the client inserts the images as clips on a "B-Roll" overlay track at each topic's time. Gated on `PEXELS_API_KEY`.
- [x] **AI Dubbing** (video + audio inspector) — transcribe → translate (OpenRouter chat) → TTS the target language (async audio generation) → place the dubbed audio on a "Dub" track and mute the original (`POST .../dub`). All OpenRouter, no extra key; language via a prompt.
- [x] **AI Tools** all wired — the remaining "AI Transitions" row is the morph generator, reachable from the Transitions panel's "Create new AI Transition"
...
- [x] **Create new AI Transition** — pick two video clips + a prompt; the server grabs each clip's boundary frame and runs a morph model async (`POST .../transition-morph` → `run_transition_morph` → Replicate), then the client drops the morph video on a "Transitions" overlay track at the boundary. Gated on `TRANSITION_MORPH_MODEL`.
...
2. **Per-clip AI enhancement** — Clean Audio (denoise) + Remove Silences DONE (ffmpeg, `POST .../enhance`). Green Screen DONE (ffmpeg chromakey). Background removal DONE for **images** (sync, `POST .../remove-bg`) **and video** (async worker path, `POST .../clip-op` → `run_clip_op` → Replicate RVM, polled via the Execution). Both gated on `REPLICATE_API_KEY`. **Eye Contact DONE** (async, operator-supplied model via `EYE_CONTACT_MODEL`). The async path (`clip_ops.run_op` + `run_clip_op`) is the reusable foundation for any slow ML op — **Remove Filler Words DONE** for video too (ffmpeg, extends `.../magic-cut`). **Face Filter DONE** (async, `FACE_FILTER_MODEL`). **AI Background Expand DONE** (async, `BACKGROUND_EXPAND_MODEL`). **Magic B-Roll DONE** (transcribe → stock photo per topic → client inserts overlay clips, `POST .../broll`, gated on `PEXELS_API_KEY`). **AI transition morph DONE** (async, two boundary frames → `run_morph` via `TRANSITION_MORPH_MODEL`; client inserts the morph clip). **AI Dubbing DONE** (transcribe → translate → TTS → dub track + mute original, `POST .../dub`, all OpenRouter). **Talking Characters DONE** (portrait + script → talking-head via `run_talking` + `TALKING_CHARACTER_MODEL`). Every AI-tools / gen feature is now wired; anything left is provider-key config or non-AI polish.
