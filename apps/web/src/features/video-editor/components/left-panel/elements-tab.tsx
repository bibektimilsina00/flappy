...
const EMOJIS = ["😂", "😍", "🔥", "✨", "👍", "🎉", "💯", "❤️"];
const SHAPES = [
  { id: "square", icon: Square, color: "#ff4c45" },
  { id: "rounded", icon: Square, color: "#3cadff" },
  { id: "circle", icon: Circle, color: "#ffd646" },
      ...
      <div className="space-y-8">
        {show("Stickers") ? (
          <Section title="Stickers">
            <div className="grid grid-cols-4 gap-2.5">
              {EMOJIS.map((e) => (
                <button key={e} type="button" className="grid aspect-square place-items-center rounded-lg bg-secondary/60 text-3xl transition-colors hover:bg-accent" title="Emoji">
                  {e}
                </button>
              ))}
            </div>
        ...
        {show("Shapes") ? (
          <Section title="Shapes">
            <div className="grid grid-cols-4 gap-2.5">
              {SHAPES.map((s) => (
                <button key={s.id} type="button" className="grid aspect-square place-items-center rounded-lg bg-secondary/60 transition-colors hover:bg-accent" title={s.id}>
