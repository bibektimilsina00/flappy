...
export default function Page() {
  ...
  // Back from Dodo checkout: land on billing and refresh while webhooks settle.
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl gap-10 px-6 py-12">
      {/* settings rail */}
      <nav className="w-52 shrink-0">
        <h1 className="px-3 text-xl font-bold tracking-tight">Settings</h1>
        <div className="mt-5 flex flex-col gap-1">
          {TABS.map(({ id, label, Icon }) => (
            ...
            >
              <Icon className={cn("size-4", tab === id && "text-teal-300")} />
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="min-w-0 flex-1">
        {tab === "account" ? <AccountTab /> : tab === "billing" ? <BillingTab /> : <ConnectionsTab />}
      </div>
    </div>
  );
}
