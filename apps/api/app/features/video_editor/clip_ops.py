...
# op -> (replicate model, input field, output ext/mime, clip kind it consumes)
_OPS = {
...
def run_op(storage, op: str, src_key: str, workspace_id, timeout_s: float = 90.0) -> tuple[str, str]:
    ...
    if not settings.replicate_api_key:
        raise RuntimeError("Background removal is not configured")
    ...
    with httpx.Client(timeout=120) as client:
        res = client.post(
            f"https://api.replicate.com/v1/models/{spec['model']}/predictions",
